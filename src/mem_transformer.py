...
class MultiHeadAttn(nn.Module):
    ...
    def forward(self, h, attn_mask=None, mems=None,
        ...
        head_k = head_k.view(c.size(0), c.size(1), self.n_head, self.d_head)
        head_v = head_v.view(c.size(0), c.size(1), self.n_head, self.d_head)
        # q_norm = torch.mean(torch.linalg.norm(head_q, dim=3, ord=2))
...
class KNNMultiHeadAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0, 
                 pre_lnorm=False):
        super(KNNMultiHeadAttn, self).__init__()
...
class RBFMultiHeadAttn(nn.Module):
    ...
    def compute_weights(self, w_init, x, attn_mask=None):
        ...
        # w_init = w_init.to(device='cuda:'+str(torch.cuda.current_device()))

        obj_2 = torch.einsum('ij,ijbn->ibn', (w_init, rbf))
        obj_3 = torch.einsum('ij,jbn->ibn', (w_init, torch.einsum('ij,ijbn->ibn', (w_init, rbf))))
        # obj_3 = torch.einsum('ijbn,ijbn,ijbn->ibn', (w_init, w_init, rbf)) # can be optimized?
        # obj_3 = obj_3[None, :, :]
    ...
    for div_val in [1, 2]:
        for d_embed in [200, 100]:
            model = MemTransformerLM(
                ...
                attn_type=8, update_mode='hard').to(device)
...
# TODO: 1. write notes, how to apply NW into transformer; 2. try to run this experiment
