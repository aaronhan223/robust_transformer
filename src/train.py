...
parser.add_argument('--attn_type', type=int, default=9,
        ...
        if args.batch_chunk > 1:
            ...
            for i in range(args.batch_chunk):
                ...
                if args.attn_type in [200,] and args.update_mode == 'rbf':
                    pi0_val = model.get_pi0()
                    pi0_val = torch.stack(pi0_val)
                    pi_norm = (pi0_val ** 2).sum()

                    pi1_val = model.get_pi1()
                    pi1_val = torch.stack(pi1_val)
                    pi_norm = pi_norm + (pi1_val ** 2).sum()
                    
                    md_val = model.get_mu_diff()
                    md_val = torch.stack(md_val)
                    md_norm = md_val.sum()
                    
                    if args.fp16:
                        optimizer.backward(loss + args.pi_reg * pi_norm + args.md_reg * md_norm)
                    else:
                        (loss + args.pi_reg * pi_norm + args.md_reg * md_norm).backward()
                else:
                    if args.fp16:
